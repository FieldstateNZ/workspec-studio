...
// TODO(A6, #39): this is a private internal COPY of aspire-hosting-c4's WorkspecC4CliRunner —
// deliberately duplicated per the A4 plan rather than reaching into aspire-hosting-c4 (that project
// is frozen pending A6's consolidation into aspire-hosting-core, tracked by #39). Keep any bugfix
// here mirrored there (and vice versa) until that consolidation lands.

/// <summary>
/// Internal process-execution and diagnostics-formatting primitives shared by every workspec-decisions
...
/// </summary>
internal static class WorkspecDecisionsCliRunner
{
    /// <summary>
    /// Upper bound on a single CLI run. Dashboard commands run synchronously from the user's
    /// perspective, so a hung CLI without this bound would wedge the command in the dashboard
    /// indefinitely instead of degrading to a reported failure.
    /// </summary>
    ...
    /// missing CLI must never fault a caller (a dashboard command), only report as a normal failed
    /// outcome. A run exceeding <paramref name="timeout"/> (default <see cref="DefaultTimeout"/>) has
    /// its whole process tree killed and degrades to the same <c>-1</c> failure shape — never a hang,
    /// never an exception. Caller-initiated cancellation (<paramref name="cancellationToken"/>) also
    /// kills the process tree, then rethrows <see cref="OperationCanceledException"/> as usual.
    /// </summary>
    ...
    /// <summary>Parses a workspec-decisions <c>--json</c> diagnostics-array stdout payload. Empty/blank input yields an empty list.</summary>
    public static IReadOnlyList<WorkspecDecisionsCliDiagnostic> ParseDiagnostics(string stdout)
    {
        ...
        return JsonSerializer.Deserialize<WorkspecDecisionsCliDiagnostic[]>(stdout, DiagnosticsJsonOptions) ?? [];
    }
    ...
    public static string FormatValidateMarkdown(IReadOnlyList<WorkspecDecisionsCliDiagnostic> diagnostics)
    {
        ArgumentNullException.ThrowIfNull(diagnostics);

        if (diagnostics.Count == 0)
        {
            return "No diagnostics — every decision/catalog artifact is valid.";
        }
