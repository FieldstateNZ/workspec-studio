...
// this module only maps the components' two-value `theme` prop
// ('dark' | 'light') onto that package's theme identifiers and re-exports the
// pieces the components need. No token values live in this repo. Mirrors
// packages/c4-ui/src/themes.ts and packages/decision-ui/src/themes.ts exactly.
...
/** The theme names the components' `theme` prop accepts. */
 ...
 * components apply this inline on their themed root element, so the
 * WorkSpec palette is bound wherever they render — no document-level
