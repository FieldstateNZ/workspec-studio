...
import { buildPromotedRule, computeUnattributedClusters, nextRuleId } from './format.js';
  ...
  return [
    {
      ...
      description:
        'Read the current cloud estate, total spend, attribution coverage, primary dimension, allowed values, and rule count. Does not change any artifact.',
      ...
      inputSchema: closedObjectSchema(
        {
          ...
          value: {
            ...
            maxLength: 128,
            description:
              'An exact allowed value returned by get_cost_overview or list_unattributed_clusters.',
          },
        },
        ['resourceGroup', 'value'],
      ),
    ...
    },
    {
      ...
      description:
        'WRITE ACTION: persist exactly one previously previewed attribution rule to the current attribution artifact. Requires the proposalId from preview_attribution_rule, rejects stale state, and immediately updates the visible Cost workbench.',
      inputSchema: closedObjectSchema(
        {
          proposalId: {
            type: 'string',
