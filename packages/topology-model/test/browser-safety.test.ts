...
/**
 * `@workspec/topology-model`'s root entry (`src/index.ts`) must never reach
 * a Node builtin import — neither `node:`-prefixed nor bare (`'fs'`,
 ...
 * as a post-build step. Mirrors `@workspec/c4-model`'s own test exactly.
 */
describe('root entry is browser-safe', () => {
  ...
  it('sanity check: the guard itself recognizes both builtin spellings', () => {
    ...
    expect(isNodeBuiltin('@workspec/topology-schema')).toBe(false);
