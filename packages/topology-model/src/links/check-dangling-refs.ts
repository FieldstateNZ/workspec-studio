...
import type { LoadedEnvironment, LoadedResource, LoadedTopology } from '../model/loaded-artifact.types.js';
 ...
 * connection endpoints, resource placement refs, the topology's declared
 * environments, and its optional catalog ref. The placement check runs
 * regardless of whether a singleton topology was found (a resource's
 * `network`/`resourceGroup` ref is a resource-to-resource concern,
 * independent of the topology file); the other three are skipped when
 * `topology` is `null` — there's nothing to check a ref *from* without one.
 */
