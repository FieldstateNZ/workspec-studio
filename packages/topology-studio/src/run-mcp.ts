// The `mcp` subcommand: run the topology MCP server over stdio. Factored out
// of `cli.ts` for the same reason `serve.ts` is: keeps this module's own
...
export const MCP_HELP = `workspec-topology mcp — run the topology MCP server over stdio

Usage:
  workspec-topology mcp [--dir <path>]

Options:
  --dir <path>   Directory of topology artifacts to serve (default: current directory).
...
export async function runMcp(argv: string[], io: CliIO): Promise<number> {
  ...
  io.err(`Topology Studio MCP · serving ${dir} over stdio\n`);
  return 0;
}
