...
// topology artifacts. Factored out of `cli.ts` so the Express app itself
...
export const SERVE_HELP = `workspec-topology serve — run the localhost Topology Studio host

Usage:
  workspec-topology serve [--dir <path>] [--port <n>] [--host <addr>] [--mcp]

Options:
  --dir <path>    Directory of topology artifacts (.workspec/{topologies,
                  resources,environments}) to serve (default: current directory).
  ...
  --mcp           Also mount an MCP server (stateless, localhost-only) at /mcp,
                  exposing the same reads/writes/validate/resolve/reconcile/cost
                  as tools.

Serves the built client and a thin JSON API over the working tree. No
database: the artifacts under --dir are the single source of truth. With no
command at all, "serve" is the default (run "workspec-topology" bare).
`;
...
export async function runServe(argv: string[], io: CliIO): Promise<number> {
  ...
  const mcpProvider =
    values.mcp === true ? createTopologyMcpProvider(new FsRepository(resolve(dir))) : undefined;
  ...
  return new Promise<number>((resolvePromise) => {
    const server = app.listen(port, host, () => {
      ...
      io.err(`Topology Studio · serving ${dir}\n`);
