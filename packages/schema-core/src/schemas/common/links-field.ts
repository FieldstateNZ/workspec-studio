...
/**
 * Shared `links` field: an array of entries, each holding exactly one
 * `{<linkType>: <pathRef>}` pair, optionally with a `cardinality` key
 * (`{from, to, label?}`) on the same entry for relationship-style links.
 * Optional — most artifacts have none. Same shape as `@workspec/c4-schema`'s
 * `linksField`, including the superRefine-based entry validation — which
 * JSON Schema cannot express, so editors won't flag a malformed entry shape;
 * runtime Zod validation will. Copied rather than imported so this package
 * has zero `@workspec` dependencies.
 */
