...
//
// Mirrors `@workspec/decision-studio`'s `path-containment.test.ts`.
...
describe('resolveWithinRoot — Windows semantics (path.win32)', () => {
  ...
  it('resolves and accepts a normal in-root relative ref (forward slashes)', () => {
    expect(resolveWithinRoot(root, 'prod/estate.inventory.yaml', win32)).toBe(
      String.raw`C:\Users\dev\workspace\prod\estate.inventory.yaml`,
    );
  });

  it('resolves and accepts a normal in-root relative ref (backslashes)', () => {
    expect(resolveWithinRoot(root, String.raw`prod\estate.inventory.yaml`, win32)).toBe(
      String.raw`C:\Users\dev\workspace\prod\estate.inventory.yaml`,
    );
  });
});
...
describe('resolveWithinRoot — POSIX semantics (default pathImpl)', () => {
  ...
  it('rejects `..` traversal that escapes root', () => {
    expect(() => resolveWithinRoot(root, '../escape.inventory.yaml')).toThrow(RefEscapesRootError);
  });

  it('resolves and accepts a normal in-root relative ref (no regression on the happy path)', () => {
    expect(resolveWithinRoot(root, 'prod/estate.inventory.yaml')).toBe(
      '/repo/root/prod/estate.inventory.yaml',
    );
  });

  it('the escaped-ref error carries the original ref for diagnostics', () => {
    try {
      resolveWithinRoot(root, '../escape.inventory.yaml');
      expect.unreachable('expected resolveWithinRoot to throw');
    } catch (error) {
      expect(error).toBeInstanceOf(RefEscapesRootError);
      expect((error as RefEscapesRootError).ref).toBe('../escape.inventory.yaml');
    }
  });
});
