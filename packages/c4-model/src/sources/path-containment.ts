// Pure containment check used by `createFsSource`'s internal `resolveRef` to
// guarantee that every ref — however it is spelled — resolves to a path
...
// for those two shapes lets them straight through. This module closes that
// gap by verifying containment *after* resolution instead of trusting the
// ref's shape beforehand.
//
// Mirrors `@workspec/decision-studio`'s and `@workspec/cost-studio`'s
// `path-containment.ts` — this is the house's accepted per-package
// duplication for dependency-light packages (each copy is ~50 lines with no
// package-specific logic, so a shared package would cost more in an extra
// workspace dependency than it saves). Unlike those two copies, `ref` here is
// a plain `string`: `@workspec/c4-model` paths are POSIX-relative strings,
// not a branded `Ref` type.
