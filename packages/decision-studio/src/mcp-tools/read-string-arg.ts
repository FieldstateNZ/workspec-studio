 ...
 * wrong type) — `assembleMcpServer` (from `@workspec/mcp-core`) catches any
 * handler throw and converts it to an `isError` `CallToolResult`, so this is
 * a legitimate "self-validation" path for a tool handler, not an escape from
 * the no-throw convention.
 */
