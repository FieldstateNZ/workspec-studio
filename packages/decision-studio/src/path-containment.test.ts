...
describe('resolveWithinRoot — Windows semantics (path.win32)', () => {
  ...
  it('resolves and accepts a normal in-root relative ref (forward slashes)', () => {
    expect(resolveWithinRoot(root, 'examples/hosting-platform/x.decision.yaml', win32)).toBe(
      String.raw`C:\Users\dev\workspace\examples\hosting-platform\x.decision.yaml`,
    );
  });

  it('resolves and accepts a normal in-root relative ref (backslashes)', () => {
    expect(resolveWithinRoot(root, String.raw`sub\x.decision.yaml`, win32)).toBe(
      String.raw`C:\Users\dev\workspace\sub\x.decision.yaml`,
    );
  });
});
...
describe('resolveWithinRoot — POSIX semantics (default pathImpl)', () => {
  ...
  it('rejects `..` traversal that escapes root', () => {
    expect(() => resolveWithinRoot(root, '../escape.decision.yaml')).toThrow(RefEscapesRootError);
  });

  it('resolves and accepts a normal in-root relative ref (no regression on the happy path)', () => {
    expect(resolveWithinRoot(root, 'examples/hosting-platform/x.decision.yaml')).toBe(
      '/repo/root/examples/hosting-platform/x.decision.yaml',
    );
  });

  it('the escaped-ref error carries the original ref for diagnostics', () => {
    try {
      resolveWithinRoot(root, '../escape.decision.yaml');
      expect.unreachable('expected resolveWithinRoot to throw');
    } catch (error) {
      expect(error).toBeInstanceOf(RefEscapesRootError);
      expect((error as RefEscapesRootError).ref).toBe('../escape.decision.yaml');
    }
  });
});
